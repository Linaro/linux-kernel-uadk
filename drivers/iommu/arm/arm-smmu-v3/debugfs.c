...
struct arm_smmuv3_reg {
	const char *name;
	u32 offset;
};
static const struct arm_smmuv3_reg regs[] = {
	{ .name = "IDR0", .offset = ARM_SMMU_IDR0 },
	{ .name = "IDR1", .offset = ARM_SMMU_IDR1 },
	{ .name = "IDR3", .offset = ARM_SMMU_IDR3 },
	{ .name = "IDR5", .offset = ARM_SMMU_IDR5 },
	{ .name = "CR0", .offset = ARM_SMMU_CR0 },
	{ .name = "CR1", .offset = ARM_SMMU_CR1 },
	{ .name = "CR2", .offset = ARM_SMMU_CR2 },
	{ .name = "STATUSR", .offset = ARM_SMMU_STATUSR },
	{ .name = "GBPA", .offset = ARM_SMMU_GBPA },
	{ .name = "IRQ_CTRL", .offset = ARM_SMMU_IRQ_CTRL },
	{ .name = "GERROR", .offset = ARM_SMMU_GERROR },
	{ .name = "GERRORN", .offset = ARM_SMMU_GERRORN },
	{ .name = "CMDQ_BASE", .offset = ARM_SMMU_CMDQ_BASE },
	{ .name = "CMDQ_PROD", .offset = ARM_SMMU_CMDQ_PROD },
	{ .name = "CMDQ_CONS", .offset = ARM_SMMU_CMDQ_CONS },
	{ .name = "EVTQ_BASE", .offset = ARM_SMMU_EVTQ_BASE },
	{ .name = "EVTQ_PROD", .offset = ARM_SMMU_EVTQ_PROD },
	{ .name = "EVTQ_CONS", .offset = ARM_SMMU_EVTQ_CONS },
	{ /* sentinel */}
};
...
static void regs_dump(struct seq_file *m, struct arm_smmu_device *smmu)
{
	u32 val;
	int i;

	seq_printf(m, "SMMUv3.%s registers:\n", dev_name(smmu->dev));
	for (i = 0; i < ARRAY_SIZE(regs) - 1; i++) {
		if (regs[i].offset > SZ_64K)
			val = readl_relaxed(smmu->page1 + regs[i].offset -
					    SZ_64K);
		else
			val = readl_relaxed(smmu->base + regs[i].offset);

		seq_printf(m, "%s: 0x%016x\n", regs[i].name, val);
	}
}
...
void arm_smmu_debugfs_init(void)
{
	...
	debugfs_create_file("regs", 0444, arm_smmu_debug, NULL, &regs_fops);
}
