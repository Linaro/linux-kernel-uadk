				 ...
				 int *pasid)
{
	...
	sva->handle = iommu_sva_bind_group(group->iommu_group, 0);
				  ...
				  struct vfio_group *group)
{
	...
	list_for_each_entry(sva, &old_group->sva_handles, next) {
		/*
		 * The WARN_ON is temporary, to catch any old users of remote
		 * binding in my test setup.
		 */
		if (WARN_ON(sva->mm != current->mm))
			goto out_unbind;

		ret = vfio_iommu_bind_group(iommu, group, NULL);
		if (ret)
			goto out_unbind;
	}
	return 0;
...
static long vfio_iommu_type1_bind(struct vfio_iommu *iommu, unsigned long arg)
{
	...
	list_for_each_entry(domain, &iommu->domain_list, next) {
		list_for_each_entry(group, &domain->group_list, next) {
			ret = vfio_iommu_bind_group(iommu, group, &new_pasid);
			if (ret)
				goto out_unbind;
